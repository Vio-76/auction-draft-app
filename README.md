...
- **Spectator board** — `/board`: read-only grid of every team, the player pool, and
  the opening-bid turn order.
- **Admin console** — `/admin`: manage players/captains, import a playerlist, control
  the auction (start/pause/reset, turn, bidding, sell), and edit rosters live.
...
2. **BIDDING** — open to all captains until the player is sold: in **MANUAL** mode the
   admin clicks Sold; in **AUTO** mode the server sells to the high bidder after a
   no-new-bid countdown. The player goes to the highest bidder and the turn advances.
